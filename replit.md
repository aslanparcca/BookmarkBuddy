...
- June 17, 2025. Enhanced "Güncel Bilgiler" section with comprehensive Google Search API interface matching user's HTML template: implemented pricing alert with 4x extra credit calculation, added search source options (Google Organik Arama, Google News, Kendi Linklerimi Gireceğim), configured search query types (focus keyword, title, custom), included country/language/date filtering controls, created excluded URLs and custom URLs management with detailed instructions, added source links display options (Eklenmesin, URL, Link Başlığı, URL + Link Başlığı, Link Başlığı + URL) and structure controls, updated all dropdown text to match exact Turkish terminology from reference images
```
