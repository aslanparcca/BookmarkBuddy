...
export default function ArticlesNew() {
  ...
  const { data: responseData, isLoading, refetch, error } = useQuery({
    queryKey: ['/api/articles', { limit: perPage, offset: (currentPage - 1) * perPage, search: searchQuery }],
  ...
  // Categories based on selected website
  const categories = selectedWebsite 
    ? websites.find(w => w.id.toString() === selectedWebsite)?.categories || []
    : [];

  // Bulk delete selected articles mutation
  ...
  // Bulk delete all articles mutation
  ...
  // Send articles to website mutation
  const sendArticlesMutation = useMutation({
    mutationFn: async ({ articleIds, websiteId, category, status }: { 
      ...
      return await apiRequest('/api/send-articles-to-website', {
        method: 'POST',
        body: JSON.stringify({
          articleIds,
  ...
  return (
    <>
      ...
      <div className="mb-4 flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">İçeriklerim</h1>
        <Button 
          onClick={() => refetch()} 
          variant="outline" 
                ...
                <TableBody>
                  {filteredArticles.map((article: any, index: number) => (
                    <TableRow key={article.id} className="hover:bg-slate-50">
                      ...
                      <TableCell className="text-center">
                        <span className="text-sm text-slate-600">{formatDate(article.createdAt)}</span>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
