...
export default function GoogleAdsTitle() {
  ...
  const generateMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest("/api/generate-google-ads-title", "POST", data);
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "Başarılı!",
        description: "Google Ads başlıkları oluşturuldu.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!product.trim() || !targetKeywords.trim()) {
      toast({
        title: "Eksik Bilgi",
        description: "Lütfen ürün/hizmet ve hedef anahtar kelimeleri girin.",
        variant: "destructive",
      });
      return;
    }
  ...
  const downloadAsText = () => {
    ...
    a.href = url;
    a.download = "google-ads-basliklari.txt";
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          Google Ads Başlığı Üretici
        </h1>
        <p className="text-gray-600 dark:text-gray-300">
          Dönüşüm odaklı Google Ads başlıkları oluşturun
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Google Ads başlıkları için gerekli bilgileri girin
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Örn: Online İngilizce kursu, LED TV, Diş tedavisi"
                  className="mt-1"
                />
              </div>

              <div>
                <Label htmlFor="keywords">Hedef Anahtar Kelimeler *</Label>
                <Textarea
                  id="keywords"
                  value={targetKeywords}
                  onChange={(e) => setTargetKeywords(e.target.value)}
                  placeholder="Örn: ingilizce kursu, online eğitim, uygun fiyat"
                  className="mt-1"
                  rows={3}
                />
              </div>
              ...
              <div>
                ...
                <Select value={tone} onValueChange={setTone}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="confident">Güvenli</SelectItem>
                  </SelectContent>
                </Select>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Google Ads için optimize edilmiş başlıklar
            </CardDescription>
          </CardHeader>
          <CardContent>
            {result ? (
